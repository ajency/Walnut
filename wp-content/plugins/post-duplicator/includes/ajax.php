 ...
 * @since 2.14
 */
function m4c_duplicate_post($original_id, $chapter, $sections, $subsections) {
	  ...
	  if( is_array($value) && count($value) > 0 ) {
			foreach( $value as $i=>$v ) {
				if( $key != '_edit_lock' && $key != '_wp_old_slug' ) {
					if($key=='term_ids'){
						$tmp = maybe_unserialize($v);
						$tmp['chapter']=$chapter;
						$tmp['sections']=$sections;
						add_post_meta( $duplicate_id, $key, $tmp);
					}
					else if($key=='content_piece_meta'){
						$tmp = maybe_unserialize($v);
						$tmp['term_ids']['chapter']=$chapter;
						$tmp['term_ids']['sections']=$sections;
						add_post_meta( $duplicate_id, $key, $tmp);
					}
					else{
