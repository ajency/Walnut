...
function my_profile() {
	...
	if(isset($_POST['student_action']) && $_POST['student_action'] == 'update-my-profile') {
		...
		$_POST['source_domain']='walnut.synapsedu.info';
...
function student_redirect(){
	$current_user = wp_get_current_user();	
	$meta = get_user_meta($current_user->ID);
	if($current_user->ID!=0 && !isset($meta['student_division']) && !strpos($_SERVER['REDIRECT_URL'],'register-redirect-student')){
	    wp_redirect(site_url().'/register-redirect-student');
	    exit;
	}/*else if($current_user->ID == 0  && !strpos($_SERVER['REDIRECT_URL'],'social-login-student')){
