...
(App, Template)->
    ...
    (Views, App, Backbone, Marionette, $, _)->
        class Views.OptionView extends Marionette.ItemView
            ...
            onShow : ->
                console.log(@hotspotModel.get('transparent'))
                if @hotspotModel.get 'transparent'
                    @$el.find('.option-color').hide()
                    @$el.find('.option-color').removeClass('inline')

                @listenTo @hotspotModel, 'change:transparent',(model,transparent)=>
                    if transparent
                        @$el.find('.option-color').hide()
                        @$el.find('.option-color').removeClass('inline')
                    else
                        @$el.find('.option-color').show()
                        @$el.find('.option-color').addClass('inline')
