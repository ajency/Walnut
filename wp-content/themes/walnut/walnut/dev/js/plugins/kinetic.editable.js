...
Kinetic.EditableText.prototype = {
	...
	focus: function(lays) {
		try {
			this.staticLayer = lays;//this.getLayer(); //changed by elroy----pass a layer on focus
		}
		catch (e) {
		...
		var obj = this.stage.getIntersection(this.stage.getAbsolutePosition());
	},
	...
	findCursorPosFromClick: function(secondary) {
		...
		$.each(this.tempText, function(index, iterTempText) {
			...
			var pos = that.stage.getAbsolutePosition();
