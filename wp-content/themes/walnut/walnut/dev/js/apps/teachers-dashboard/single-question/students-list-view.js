...
define(['app'], function(App) {
  return App.module("TeachersDashboardApp.View.StudentsList", function(StudentsList, App) {
    ...
    return StudentsList.List = (function(_super) {
      ...
      List.prototype.selectStudent = function(e) {
        return $(e.target).closest('.tiles.single').toggleClass("selected");
      };

      List.prototype.markAsCorrectAnswer = function() {
        ...
        for (_i = 0, _len = selectedStudents.length; _i < _len; _i++) {
          ...
          $(student).removeClass('selected').find('.default').removeClass('default').addClass('green').find('i').removeClass('fa-minus-circle').addClass('fa-check-circle');
        }
        console.log(_.uniq(this.correctAnswers));
        return this.trigger("save:question:response");
      };

      List.prototype.markAsWrongAnswer = function() {
        var selectedStudents, student, _i, _len;
        selectedStudents = this.$el.find('.tiles.single.selected');
        for (_i = 0, _len = selectedStudents.length; _i < _len; _i++) {
          student = selectedStudents[_i];
